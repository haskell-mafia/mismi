...
import           Network.HTTP.Client (HttpException (..), HttpExceptionContent (..), responseTimeoutMicro, responseStatus)
...
runAWS e'' =
  let e' = over envManager (\m -> m { mResponseTimeout = responseTimeoutMicro 60000000 }) e''
