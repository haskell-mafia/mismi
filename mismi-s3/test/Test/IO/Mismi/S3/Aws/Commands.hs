...
module Test.IO.Mismi.S3.Aws.Commands where
...
prop_exists_compatibility :: Bool -> Property
...
prop_list_recursively_compatibility :: Property
prop_list_recursively_compatibility = withAWS $ \a -> do
  write a ""
  r <- listRecursively (a { key = dirname $ key a })
  r' <- A.listRecursively (a { key = dirname $ key a })
  pure $ r === r'
